...
function extract_beta!(flat::Matrix)
    for i = 1:size(flat)[2]
        k1 = flat[3,i]; k2 = flat[4,i];
        flat[3,i] = k1/(k1+k2)
        flat[4,i] = (k1*k2)/((k1+k2)^2*(k1+k2+1))
    end
    return flat
end
...
x = vcat(rand(pri_gamma,2,numwalkers),rand(pri_beta1,1,numwalkers),rand(pri_beta2,1,numwalkers),rand(pri,numdims-4,numwalkers));
